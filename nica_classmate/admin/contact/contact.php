...
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM contact {$whereClause}");
$countStmt->execute($params);
$totalRecords = $countStmt->fetchColumn();
$totalPages = ceil($totalRecords / $perPage);

$stmt = $pdo->prepare("SELECT * FROM contact {$whereClause} ORDER BY created_at DESC LIMIT {$perPage} OFFSET {$offset}");
$stmt->execute($params);
...
<head>
    ...
    <title>Contact Management - Click by Nica</title>
            ...
            <div class="p-6 border-b border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 mt-1">Total: <?php echo $totalRecords; ?> messages</p>
                    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900"><?php echo htmlspecialchars(substr($contact['message'], 0, 100)); ?><?php echo strlen($contact['message']) > 100 ? '...' : ''; ?></div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <a href="view.php?id=<?php echo $contact['id']; ?>" class="text-indigo-600 hover:text-indigo-900">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="edit.php?id=<?php echo $contact['id']; ?>" class="text-yellow-600 hover:text-yellow-900">
